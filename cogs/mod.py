...
allowed_mentions_config = discord.AllowedMentions(everyone=False, users=True, roles=False, replied_user=False)
...
class mod(commands.Cog):
    ...
    g_mod = app_commands.Group(name="mod", description="MODERATORS command")
    
    @g_mod.command(name="ban", description="Ban User mà bạn muốn ban")
    ...
    @g_mod.command(name="unban", description="Unban User mà bạn muốn gỡ ban")
    @app_commands.default_permissions(ban_members=True)
    ...
    @g_mod.command(name="kick", description="Kick menber của bạn")
    ...
    @g_mod.command(name="timeout", description="Set timeout cho menber của bạn")
    ...
    @g_mod.command(name="untimeout", description="Bỏ timeout cho menber của bạn")
    @app_commands.default_permissions(moderate_members=True)
    ...
    @g_mod.command(name="add_role", description="Thêm role cho menber của bạn")
    @app_commands.default_permissions(manage_roles=True)
    async def add_role(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role, reason:str=nors):
        ctx = await Interactx(interaction)
        member = await ctx.guild.fetch_member(member.id)
        mess = await ctx.send(f"**Đang Check User**")
        if member.get_role(role.id):
            await mess.edit(content=f"**{botemoji.no} Người đã có role {role.mention}**", allowed_mentions=allowed_mentions_config)
            return
        if (((member.top_role.position >= ctx.author.top_role.position) or (role.position >= member.top_role.position)) and ctx.author.id != ctx.guild.owner_id) or member.id == ctx.guild.owner_id:
            await mess.edit(content=f"**{botemoji.no} Bạn không thể thêm role {role.mention} cho người này**", allowed_mentions=allowed_mentions_config)
            return
        async def run():
            roleuserout = False
            try:
                await member.add_roles(role, reason=f"Add role by {ctx.author} | Reason: {reason}")
                roleuserout = True
            except discord.Forbidden:
                await mess.edit(content=f"**{botemoji.no} Bot không có đủ quyền để thêm role cho người này**")
            if roleuserout:
                await mess.edit(content=f"**{botemoji.yes} Đã thêm role {role.mention} cho {member}**", allowed_mentions=allowed_mentions_config)
        view = Confirm(run(), ctx)
        await mess.edit(content=f"**Hãy nhấn {botemoji.yes} thêm bỏ role {role.mention} của {member}**", view=view, allowed_mentions=allowed_mentions_config)
        await view.wait()
        if view.value is None:
            await mess.edit(content=f"**{botemoji.no} Lệnh bị hủy do không có phản hồi**", view=None)
            
    @g_mod.command(name="remove_role", description="Bỏ role cho menber của bạn")
    @app_commands.default_permissions(manage_roles=True)
    async def remove_role(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role, reason:str=nors):
        ctx = await Interactx(interaction)
        member = await ctx.guild.fetch_member(member.id)
        mess = await ctx.send(f"**Đang Check User**")
        if not member.get_role(role.id):
            await mess.edit(content=f"**{botemoji.no} Người không có role {role.mention}**", allowed_mentions=allowed_mentions_config)
            return
        if (((member.top_role.position >= ctx.author.top_role.position) or (role.position >= member.top_role.position)) and ctx.author.id != ctx.guild.owner_id) or member.id == ctx.guild.owner_id:
            await mess.edit(content=f"**{botemoji.no} Bạn không thể thêm role {role.mention} cho người này**", allowed_mentions=allowed_mentions_config)
            return
        async def run():
            roleuserout = False
            try:
                await member.remove_roles(role, reason=f"Add role by {ctx.author} | Reason: {reason}")
                roleuserout = True
            except discord.Forbidden:
                await mess.edit(content=f"**{botemoji.no} Bot không có đủ quyền để bỏ role cho người này**")
            if roleuserout:
                await mess.edit(content=f"**{botemoji.yes} Đã bỏ role {role.mention} cho {member}**", allowed_mentions=allowed_mentions_config)
        view = Confirm(run(), ctx)
        await mess.edit(content=f"**Hãy nhấn {botemoji.yes} thêm bỏ role {role.mention} của {member}**", view=view, allowed_mentions=allowed_mentions_config)
        await view.wait()
        if view.value is None:
