...
class vh(commands.Cog):
    ...
    async def on_voice_state_update(self, member:discord.Member, before:discord.VoiceState, after:discord.VoiceState):
        if before.channel != after.channel:
            if before.channel is not None:
                async with database(f"./data/tempvoice/{member.guild.id}", self.bot.db) as db:
                    if str(before.channel.id) in (await db.keys()):
                        if len([ i for i in before.channel.members if not i.bot]) == 0:
                            await db.remove(str(before.channel.id))
                            try:
            ...
            if after.channel is not None:
                async with database(f"./data/voicehub/{member.guild.id}", self.bot.db) as db:
                    if str(after.channel.id) in (await db.keys()) and (not member.bot):
                        edit_dict = {}
                        main_cf = await db.get(str(after.channel.id))
                        ...
                        if main_cf["type"] == "vc":
                            ...
                            if main_cf.get("full_control"):
                                ...
                                pr["manage_channels"] = True
                                pr["move_members"] = True
                            await ch.set_permissions(member, view_channel=True, connect=True, **pr)
                            if main_cf.get("bitrate") is not None:
                                if (int(member.guild.bitrate_limit) / 1000) < main_cf["bitrate"]:
                                    a = await db.get(str(after.channel.id))
                                    a["bitrate"] = (int(member.guild.bitrate_limit) / 1000)
                                    await db.set(str(after.channel.id), a)
                        ...
                        async with database(f"./data/tempvoice/{member.guild.id}", self.bot.db) as tdb:
                            await tdb.set(str(ch.id), {"owner": member.id, "prcf": main_cf})
                            try:
                                await member.move_to(ch, reason=f"VoiceHub | Move {member} To Temp Voice")
                            except BaseException:
                                await none_exit()
                                try:
                                    await db.remove(str(ch.id))
                                except BaseException:
                                    pass
                                
    @commands.Cog.listener()
    async def on_guild_channel_delete(self, channel:discord.abc.GuildChannel):
        async with database(f"./data/tempvoice/{channel.guild.id}", self.bot.db) as tdb:
            if str(channel.id) in (await tdb.keys()):
                await tdb.remove(str(channel.id))
                
        async with database(f"./data/voicehub/{channel.guild.id}", self.bot.db) as db:
            if str(channel.id) in (await db.keys()):
                await db.remove(str(channel.id))
    ...
    async def vh_create(self, interaction: discord.Interaction, name:str, v_type:app_commands.Choice[str], category:discord.CategoryChannel=None):
        ...
        try:
            ...
            await edit.edit(embed=embed)
            return
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            await db.set(str(ch.id), {"type": v_type.value})
    ...
    async def vh_cf_bitrate(self, interaction: discord.Interaction, channel:discord.VoiceChannel, bitrate:app_commands.Range[int, 8, 384]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            if str(channel.id) not in (await db.keys()):
                ...
                await ctx.send(embed=embed)
                return
            if (await db.get(str(channel.id)))["type"] == "vc":
                ...
                if bitrate != 64:
                    ...
                    if "bitrate" in (await db.get(str(channel.id))).keys():
                        a = await db.get(str(channel.id))
                        del a["bitrate"]
                        await db.set(str(channel.id), a)
            if (await db.get(str(channel.id)))["type"] == "sc":
                if bitrate > 64:
                    embed = discord.Embed(title=f"Giới hạn bitrate của Stage Channels là 64")
                    await ctx.send(embed=embed)
                    return
                if bitrate != 64:
                    a = await db.set(str(channel.id), a)
                    a["bitrate"] = bitrate
                    await db.set(str(channel.id), a)
                else:
                    if "bitrate" in (await db.get(str(channel.id))).keys():
    ...
    async def vh_cf_full_control(self, interaction: discord.Interaction, channel:discord.VoiceChannel, var:app_commands.Choice[int]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            ...
            if var.value == 1:
                ...
                a["full_control"] = True
                await db.set(str(channel.id), a)
            elif (await db.get(str(channel.id))).get("full_control") is not None:
                a = await db.get(str(channel.id))
                del a["full_control"]
                await db.set(str(channel.id), a)
    ...
    async def vh_cf_nsfw(self, interaction: discord.Interaction, channel:discord.VoiceChannel, var:app_commands.Choice[int]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            ...
            if var.value == 1:
                ...
                a["nsfw"] = True
                await db.set(str(channel.id), a)
            elif (await db.get(str(channel.id))).get("nsfw") is not None:
                a = await db.get(str(channel.id))
                del a["nsfw"]
                await db.set(str(channel.id), a)
    ...
    async def vh_cf_lock(self, interaction: discord.Interaction, channel:discord.VoiceChannel, var:app_commands.Choice[int]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            ...
            if var.value == 1:
                ...
                a["private"] = True
                await db.set(str(channel.id), a)
            elif (await db.get(str(channel.id))).get("private") is not None:
                a = await db.get(str(channel.id))
                del a["private"]
                await db.set(str(channel.id), a)
    ...
    async def vh_cf_user_limit(self, interaction: discord.Interaction, channel:discord.VoiceChannel, user_limit:app_commands.Range[int, 0, 10000]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            ...
            if user_limit != 0 :
                ...
                await db.set(str(channel.id), a)
            elif (await db.get(str(channel.id))).get("user_limit") is not None:
                a = await db.get(str(channel.id))
                del a["user_limit"]
                await db.set(str(channel.id), a)
            if user_limit == 0:
    ...
    async def vh_cf_slowmode(self, interaction: discord.Interaction, channel:discord.VoiceChannel, seconds:app_commands.Range[int, 0, 21600]):
        ...
        async with database(f"./data/voicehub/{ctx.guild.id}", self.bot.db) as db:
            ...
            if seconds != 0 :
                ...
                a["slowmode"] = seconds
                await db.set(str(channel.id), a)
            elif (await db.get(str(channel.id))).get("slowmode") is not None:
                a = await db.get(str(channel.id))
                del a["slowmode"]
                await db.set(str(channel.id), a)
    ...
    async def v_claim(self, interaction: discord.Interaction, channel:typing.Union[discord.VoiceChannel, discord.StageChannel]):
        ctx = await Interactx(interaction)
        async with database(f"./data/tempvoice/{ctx.guild.id}", self.bot.db) as db:
            if await self.check_temp(ctx, channel, db):
                return
            old_user = ctx.guild.get_member((await db.get(str(channel.id)))["owner"])
            ...
            pr = {}
            if (await db.get(str(channel.id)))["prcf"].get("full_control"):
                pr["manage_permissions"] = True
            ...
            preconfig = await db.get(str(channel.id))
            preconfig["owner"] = ctx.author.id
            await db.set(str(channel.id), preconfig)
